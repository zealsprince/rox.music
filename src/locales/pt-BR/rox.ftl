...
site-description = Um player nativo para coleções locais grandes e bem etiquetadas. Monte seus painéis, compartilhe temas como arquivos, siga rápido com 50.000 faixas.

social-image-alt = O logo do rox acima das palavras: se o Foobar2000 fosse feito nos dias de hoje
...
app-feature-workspaces = Workspaces compartilháveis que carregam layout, paleta e aparência
app-feature-tagging = Edição profunda de tags com escritas atômicas e edições em lote
app-feature-gapless = Reprodução sem intervalos
...
home-hero = Se o Foobar2000 fosse feito nos dias de hoje.
    .lede = Um player de música de desktop para quem tem coleções locais grandes e bem etiquetadas. Os painéis você monta sozinho, e um tema é um arquivo só que dá para passar para outra pessoa. Nativo em Linux, macOS e Windows.
    .alt = o rox rodando o workspace padrão no estilo NekoRoX, mostrando a biblioteca, a capa, a fila e um painel de espectro

home-speed = Menos de um segundo até uma janela
...
home-closer = Faça dele o seu
    .body = São { workspace-count } workspaces já na caixa, a um clique na janela de boas-vindas. Cada um é um arquivo único com o layout, a paleta e os shaders que ele roda, então dá para editar, quebrar e passar para outra pessoa. [Veja todos](/workspaces).
...
benchmark-never-settles = nunca assenta

benchmark-method = Medi todos eles eu mesmo, numa máquina só (Ryzen 9 5950X, Linux/Wayland) com a mesma coleção de 50 mil faixas num SSD externo, cada player pela mesma bancada com a biblioteca carregada. Memória é PSS da árvore de processos inteira. "Carregado" é quando a CPU assenta depois do início; repouso é um player pausado com a janela visível.
    .deadbeef = O [DeaDBeeF](@deadbeef) teria ficado em 44 MB e ganhado no quesito memória, mas não conseguiu uma linha: o parser de MP3 dele dá segfault no meio da importação desta coleção, em toda tentativa. O rox indexa as 50.000 faixas, incluindo os arquivos truncados que o derrubam.
...
feature-library = Uma biblioteca que aguenta
    .body = Um scanner paralelo lê as tags completas, as durações reais e o codec de cada arquivo, e não um cabeçalho passado por cima. A monitoração de pastas sobrevive a renomeações, e nada some em silêncio.
...
feature-gapless = Reprodução sem intervalos
    .body = Um stream só, com aleatório, repetição e tocar em seguida. Ele se recupera quando um dispositivo de áudio some, e as teclas de mídia e o now-playing funcionam nas três plataformas.
...
feature-similarity = Ordenado por como soa
    .body = O rox descreve suas faixas a partir do áudio delas, andamento incluído, e ordena o que vem a seguir por semelhança. Roda na sua máquina sobre os seus arquivos, sem serviço nenhum no meio.
...
feature-themes = Temas como arquivos compartilháveis
    .body = Um workspace é um arquivo só com layout, paleta, aparência e shaders. As paletas podem se tingir pela capa que toca, indo para o claro ou o escuro junto com a imagem.
    .link = Os { workspace-count } que vêm na caixa
...
download-title = Baixar o rox para Linux, macOS ou Windows
    ...
    .image-alt = O logo do rox acima das palavras: download para Linux, macOS e Windows
...
workspaces-title = Workspaces do rox - temas e layouts compartilháveis
    .description = Um workspace do rox é um visual inteiro num arquivo compartilhável: layout, paleta, aparência e os shaders que ele roda. Eles vão de uma homenagem ao Foobar2000 a um player renderizado inteiro em dither de 1 bit.
    .image-alt = O logo do rox acima das palavras: um visual inteiro num arquivo compartilhável

workspaces-lede = Um workspace é um visual inteiro num arquivo: layout, paleta, aparência e todos os shaders que ele roda, embutidos, para que um visual importado chegue completo. Carregue um pela janela de boas-vindas ou pelas configurações, mude o que quiser, e passe o arquivo para outra pessoa receber exatamente a mesma coisa. Estes { workspace-count } vêm na caixa.

workspaces-critters = Como o Critters faz isso
    .body = Um workspace carrega os shaders dele embutidos, código e imagens juntos, para que um visual que renderiza o aplicativo inteiro como uma impressão de 1 bit seja importado por inteiro em vez de chegar como referência a um arquivo na máquina de outra pessoa. Nada roda antes de você aprovar. [Shaders e sinais](/music-visualizer).

workspace-default = A cara do rox recém-instalado: superfícies translúcidas, tintura pelas capas desligada.
    .alt = O workspace Default do rox: uma grade de capas ao lado de uma playlist com avaliações em estrelas, uma barra de posição em forma de onda âmbar embaixo e abas para biografia, letra e metadados.
...
workspace-foobar = O layout com o qual este projeto inteiro discute. Opaco, denso, decorações do sistema ligadas, barra de menus onde ela sempre esteve.
    .alt = O workspace Foobar do rox: um layout clássico de foobar2000 com barra de menus, colunas de filtro por artista e álbum, uma tabela de faixas densa com contagens de reprodução e avaliações, e a capa no canto inferior esquerdo.

workspace-catrox = O skin CaTRoX de foobar2000 que você conhece e ama, o que começou tudo.
    .alt = O workspace CaTRoX do rox: o skin de foobar2000 recriado, com uma renderização circular da capa como CD, uma lista de campos de metadados descendo à esquerda e faixas agrupadas por álbum com pontos de avaliação.

workspace-llama = O WinAmp, do jeito que você lembra e não do jeito que ele era. Tahoma, escuro, sem moldura.
    .alt = O workspace Llama do rox: um analisador de espectro pontilhado no estilo Winamp no topo, controles de transporte verdes e uma lista de faixas densa embaixo, sem moldura de janela.
...
workspace-metro = Painéis planos e linhas folgadas com a tintura pelas capas ligada, então a paleta segue a capa que estiver tocando.
    .alt = O workspace Metro do rox: uma grade de capas de álbum ocupando a largura inteira com os títulos embaixo, e à direita um painel de biografia do artista com foto, tags de gênero e contagem de ouvintes.

workspace-phosphor = Monoespaçado em tudo. Consolas, sem capa na reprodução rápida, um terminal que por acaso toca música.
    .alt = O workspace Phosphor do rox: verde no preto em monoespaçado do começo ao fim, uma árvore de pastas ao lado de uma tabela de faixas e uma forma de onda verde embaixo, como um terminal que toca música.
...
workspace-critters = O aplicativo inteiro como uma impressão de 1 bit. Um dither ordenado sobre cada superfície, tons que esmagam com o sub-grave, e uma parede de ruído atrás dos painéis que se move com a música.
    .alt = O workspace Critters do rox: a janela inteira renderizada em dither ordenado preto e branco, com uma prateleira curva de capas sobre uma barra de posição, uma fileira de transporte, uma tira de forma de onda e uma tabela de faixas, tudo em cima de um campo de ruído que se desloca.
...
workspace-diffuse = Só o álbum que está tocando, sobre um banho suave da própria capa dele. Monocromático de propósito, para que a cor venha da imagem, e a biblioteca espera numa gaveta na borda direita.
    .alt = O workspace Diffuse do rox: uma capa de álbum grande preenchendo a esquerda da janela contra um gradiente desfocado da mesma imagem, com o título, o artista e o álbum ao lado sobre uma linha fina de posição, uma fileira de botões de transporte e um controle de volume.
...
more-why = Por que elas estão aqui
    .p1 = Estas páginas existem para serem encontradas. Quem procura uma alternativa ao Foobar2000 no Linux em { $year }, ou está tentando descobrir qual player sobrevive a uma coleção deste tamanho, deveria cair em algum lugar que responda, e não numa página inicial escrita para quem já sabe o que o rox é.
    .p2 = Elas estão fora da navegação de propósito. O site principal são três páginas: o que o rox é, como conseguir, com que cara ele fica. Empilhar o resto no cabeçalho tornaria mais difícil achar aquilo que você veio buscar, o que é uma troca ruim para todo mundo que chegou já sabendo.
    .p3 = Talvez você já saiba tudo isso sobre o rox e o Foobar2000. Muita gente que chega aqui não sabe, e é para essas pessoas que estas páginas existem. Cada uma diz onde o rox perde, e não só onde ele ganha, e não repete nada do que você já leria na [página inicial](/).
...
linux = rox - um player de música nativo para Linux
    .description = Um player de música nativo de Linux para coleções locais grandes. No AUR e como flake do Nix, Wayland e X11, saída exclusiva por ALSA, teclas de mídia via MPRIS, e menos de um segundo até uma janela com 50.000 faixas.
    .h1 = Um player de música para Linux que foi feito para o Linux
    .lede = Não é uma porta, não é um empacotamento em Electron, e não é o Foobar2000 no Wine. Um binário nativo que trata o Linux como alvo de primeira classe e não como o terceiro.
...
linux-why-1 = Os bons players de Linux abrem mão de algo cada um. O tagging do Tauon é raso. O Strawberry não é modular. Ninguém montaria um tema para o Quod Libet. A combinação de painéis, tagging profundo e uma cultura de temas ao mesmo tempo está presa no Windows há vinte anos.
...
linux-display-1 = Os dois, nativamente, pelo gpui. As decorações de janela são uma configuração por workspace e não uma decisão de tempo de compilação, então dá para rodar decorações do servidor num layout e um sem borda no próximo, sem reiniciar noutra build.
...
linux-desktop-1 = As teclas de mídia e o now-playing passam por um serviço MPRIS de verdade, então o playerctl, os widgets de mídia do GNOME e do KDE, e qualquer outra coisa que fale `org.mpris.MediaPlayer2` enxergam tudo sem configuração.
linux-desktop-2 = O ícone da bandeja vai para o barramento por StatusNotifierItem, e é isso que torna o fechar-para-a-bandeja seguro: se não houver nenhum host SNI rodando para recebê-lo, fechar a última janela encerra em vez de deixar um processo sem interface do qual você não consegue voltar.

linux-benchmarks = Os benchmarks são só de players de Linux
linux-benchmarks-1 = A tabela de comparação deste site foi rodada no Linux contra software de Linux: Elisa, fooyin, Quod Libet, Strawberry, Museeks e Tauon, todos numa máquina só com a mesma coleção de 50.000 faixas. O rox coloca uma janela na tela em 0,3 segundo e assenta em 134 MB. O mais leve depois dele que chega a terminar de carregar usa quase o triplo de memória.
...
windows-wasapi-1 = O modo exclusivo toma o endpoint em WASAPI exclusive, o que coloca a taxa e a profundidade de bits do próprio arquivo na frente do conversor sem nada no meio. O rox informa de volta o que o dispositivo realmente aceitou, então se o hardware recusou seus 192 kHz caladinho, você descobre por uma leitura na tela em vez de por uma discussão de fórum.
...
windows-integration-1 = As teclas de mídia e a sobreposição de now-playing passam pelo SMTC, a mesma superfície de sistema que os aplicativos internos usam, então o painel da tecla de volume mostra a faixa e os botões de transporte funcionam.
windows-integration-2 = Tem ícone de bandeja com fechar-para-a-bandeja, então fechar a última janela deixa a música tocando em vez de encerrá-la, e o ícone é o caminho de volta. Um workspace também pode tirar a moldura da janela, ou manter a moldura e abrir mão só da borda de redimensionamento, o que deixa a sombra, os snap layouts e o Win+seta intactos e sacrifica apenas o arraste pela borda.
...
best-shortlist = A lista curta honesta
    ...
    .aside = Os cinco critérios acima são o piso, não o teto. As avaliações vão para os próprios arquivos por FMPS e POPM em vez de um banco de dados que você não pode levar, e os gêneros com vários valores são unificados como uma opinião da biblioteca em vez de uma reescrita das suas tags. Se é esse tipo de coisa que você vinha querendo de uma biblioteca, é este que você aponta para o seu disco.
...
musicbee-good = No que o MusicBee é bom
    .p1 = Amplitude. Ele gerencia uma coleção grande direito, dá conta de playlists automáticas e automação de fila, sincroniza com aparelhos portáteis, faz podcasts e rádio pela internet, converte formatos, e pode ser skinado em algo que você goste de olhar. É gratuito, é polido, e para muita gente é o último player de música que precisou instalar.
...
musicbee-transfers = O que vem junto, e o que é melhor
    .p1 = A biblioteca aguenta o tamanho que os usuários de MusicBee costumam ter. Um scanner paralelo lê as tags completas, as durações reais e o codec, a taxa de amostragem e a profundidade de bits de cada arquivo em vez de passar por cima dos cabeçalhos, e arquivos com tags ilegíveis são indexados pelo nome em vez de desaparecerem em silêncio. A edição de tags é um editor de verdade com edições em lote, uma grade por arquivo, e um [caminho de escrita que copia e verifica](/mp3tag-alternative) em vez de modificar os originais no lugar.
    .p2 = A interface é a diferença maior. O MusicBee te dá um bom layout e algum skinning; o rox te dá uns quarenta tipos de painel que você mesmo arruma, duplica com configurações próprias e destaca em janelas de verdade do sistema. Um visual inteiro é um arquivo com layout, paleta e aparência, o que significa que o arranjo em que você parar é algo que dá para entregar a outra pessoa, ou levar para a sua outra máquina, em vez de algo que vive numa instalação só.
...
musicbee-size = A questão do tamanho
    .body = O MusicBee não está nesta tabela porque os benchmarks foram rodados no Linux, onde ele não roda. Ela está aqui como resposta para "a alternativa cai no tamanho da minha coleção?", medida em 50.000 faixas.
...
catrox-closer = Experimente
    .body = O workspace CaTRoX vem na caixa, a um clique na janela de boas-vindas. Outros { $others } também, incluindo um [layout Foobar e um com cara de WinAmp](/workspaces).
...
cue-what = O que um rip em cue realmente é
    ...
    .p2 = Os players resolvem isso de uma entre três maneiras. Ignorar o sheet e mostrar uma faixa de setenta minutos. Mostrar as faixas do sheet mas mantê-las separadas da biblioteca de verdade, de modo que busca, ordenação e playlists se comportam diferente com elas. Ou levar a quebra a sério e absorvê-la de uma vez.
...
cue-playing = Tocando um trecho como um arquivo
    .p1 = A engine trata um trecho como o mundo inteiro daquela faixa: uma busca precisa até o começo dele, um corte com precisão de amostra nas duas pontas, e um limite de fim que toma o mesmo caminho de um fim de arquivo de verdade. Sem intervalos, crossfade, parar depois da atual e repetir funcionam todos sem saber que trechos existem.
    .p2 = O corte da cabeça é o detalhe que importa. Uma busca precisa cai num limite de pacote e não na amostra exata, então, sem descartar os quadros entre a aterrissagem e o começo do trecho, toda faixa abre com o rabo da anterior. É esse o som de uma implementação de cue que ninguém terminou.
...
cue-later-scrobble = O scrobbling e o now-playing deduplicam no par, então as onze faixas de um disco viram onze scrobbles em vez de uma bem longa.
cue-later-listens = As audições se reagarram por trecho depois de uma nova varredura, então um disco mantém o histórico de reprodução por faixa mesmo quando todos os trechos carregam tags idênticas.
...
fb-does = O que o rox faz a respeito
    .p1 = Nativo nas três plataformas, um binário para cada, escrito em Rust sobre [gpui](@gpui). Uns quarenta tipos de painel que você mesmo arruma, duplica com configurações independentes e destaca em janelas de verdade do sistema. O scanner da biblioteca lê tags completas, durações reais e o codec, a taxa de amostragem e a profundidade de bits de cada arquivo em vez de passar por cima dos cabeçalhos, e ele aguenta 50.000 faixas.
    .p2 = A resposta para temas é a parte que mais mudou. Um workspace é um arquivo único carregando layout, paleta e aparência juntos. Você edita, você quebra, você entrega para outra pessoa e ela recebe exatamente o que você tinha. Um dos { workspace-count } que vêm na caixa é uma [recriação do CaTRoX](/catrox), que é a declaração honesta de intenção.

fb-shot-alt = O workspace CaTRoX no rox: o skin de foobar2000 recriado nativamente, com uma renderização circular da capa como CD, uma lista de campos de metadados à esquerda e faixas agrupadas por álbum com pontos de avaliação.
...
mp3tag-isnt = Comece pelo que isto não é
    ...
    .p2 = O argumento para fazer isso dentro do rox é que etiquetar uma coleção normalmente não é uma tarefa que se termina. É algo que você faz enquanto escuta, no arquivo que acabou de notar que estava errado, e por isso o editor estar preso ao player e à biblioteca é o ponto, e não um meio-termo.

mp3tag-write = O caminho de escrita
    .p1 = Esta é a parte que vale ler mesmo que você nunca instale o rox. A maioria dos escritores de tag modifica seu arquivo no lugar: eles abrem o original, reescrevem a região de metadados e torcem. Isso dá certo dezenas de milhares de vezes e catastroficamente errado uma, porque uma queda, um disco cheio ou um drive arrancado no meio deixam um arquivo que não abre mais, com o seu áudio dentro.
    ...
    .p3 = Cada arquivo também é isolado, então um arquivo problemático num lote de quatro mil falha sozinho em vez de derrubar a execução inteira.
...
viz-title = Uma visualização de música embutida no player - rox
    ...
    .image-alt = O logo do rox acima das palavras: sinais tirados da música, shaders em cada superfície
...
viz-signals = Os sinais são a parte diferente
    ...
    .p4 = Uma rota liga um sinal a um parâmetro com um intervalo de saída: este sinal, este alvo, daqui até ali. Dezesseis slots chegam a todo shader, e os slots para os quais nada está roteado ganham um controle simples na página de configurações, então um visual pode ser metade tocado e metade ajustado na mão.
...
viz-critters = Critters, como argumento
    .body = Um dos workspaces que vêm na caixa é o player inteiro renderizado como uma impressão de 1 bit: um dither ordenado sobre cada superfície, tons que esmagam com o sub-grave, e uma parede de ruído atrás dos painéis que se move com a música. São cinco sinais e quatro shaders num arquivo só, e nada disso é um caso especial dentro do aplicativo.
...
viz-trust = Rodando o shader de um estranho
    .p1 = Os shaders viajam dentro dos arquivos de workspace, o que significa que um visual importado chega carregando código de outra pessoa.
...
viz-closer = Experimente na sua própria coleção
    .body = Nada aqui liga para lugar nenhum: a análise é o áudio que já está passando pelo player. Mais sobre [o que um workspace carrega](/workspaces).
...
neko-rox = O que o rox faz com isso
    .body = O rox é um player nativo escrito em Rust, no Linux, no macOS e no Windows, sem foobar2000 e sem Wine por baixo. O workspace padrão dele é o layout do NekoRoX reconstruído a partir de painéis nativos, e é por isso que as duas capturas se parecem. Tem um [workspace CaTRoX](/catrox) na caixa também, junto com outros { $others }.

neko-default-alt = O workspace padrão do rox, uma reconstrução nativa do layout do NekoRoX: uma grade de capas ao lado de uma playlist com avaliações em estrelas, uma barra de posição em forma de onda âmbar embaixo, e abas para biografia, letra e metadados.

neko-carried = A lista de recursos, linha por linha
    .after = A coisa que o NekoRoX não conseguia fazer é a que mais me importa. No rox um visual inteiro é um arquivo único carregando layout, paleta e aparência juntos, então você entrega um workspace a alguém e a pessoa tem o seu setup. Sem lista de componentes, sem ordem de instalação, sem pasta de fontes.
...
neko-had-mini = Modos mini e micro player
    .now = Um layout mini por workspace, alternado pelo transporte, com tamanho de janela próprio.
...
neko-had-lastfm = Integração com Last.fm
    .now = Scrobbling, now-playing, curtidas espelhadas como loves, e importação de faixas curtidas.

neko-closer = Baixe o rox
    .body = Livre, de código aberto, um binário por plataforma. A [página de workspaces](/workspaces) mostra os { workspace-count } visuais que vêm na caixa, e a [comparação com o foobar2000](/foobar2000-alternative) cobre onde o foobar2000 ainda está na frente.
...
rg-unmeasured = Os arquivos que ninguém nunca mediu
    ...
    .p3 = Essa passagem pode se disparar sozinha. Com a medição automática ligada, os arquivos são medidos assim que a monitoração de pastas assenta depois de uma importação, e uma faixa jogada num álbum que já tinha sido medido manda o disco inteiro de volta para nova medição, já que o ganho de álbum é um número sobre o disco e não sobre o arquivo.
...
rg-limit-rate-switch = Seguir a taxa da origem no modo exclusivo custa uma pausa audível entre duas faixas de taxas de amostragem diferentes, já que o stream reabre. A reprodução sem intervalos dentro de uma mesma taxa não é afetada.
