...
social-image-alt = El logo de rox sobre las palabras: si Foobar2000 se hubiera hecho este año
...
home-hero = Si Foobar2000 se hubiera hecho este año.
    .lede = Un reproductor de música de escritorio para gente con bibliotecas locales grandes y bien etiquetadas. Los paneles los montas tú, y un tema es un archivo que le puedes pasar a otra persona. Nativo en Linux, macOS y Windows.
...
home-closer = Hazlo tuyo
    .body = Vienen { workspace-count } espacios de trabajo de serie, a un clic en la ventana de bienvenida. Cada uno es un solo archivo con la disposición, la paleta y los shaders que usa, así que puedes editarlo, romperlo y pasárselo a alguien. [Míralos todos](/workspaces).
...
benchmark-method = Todo esto lo medí yo, en una sola máquina (Ryzen 9 5950X, Linux/Wayland), con la misma biblioteca de 50.000 pistas en un SSD externo, cada reproductor con el mismo montaje y su biblioteca cargada. La memoria es PSS de todo el árbol de procesos. "Cargado" es cuando la CPU se estabiliza tras el arranque; en reposo es un reproductor en pausa con la ventana visible.
    .deadbeef = [DeaDBeeF](@deadbeef) se habría quedado en 44 MB y habría ganado en memoria, pero no llegó a tener fila: su analizador de MP3 revienta a medio importar esta biblioteca, en todos los intentos. rox indexa las 50.000 pistas, incluidos los archivos truncados con los que tropieza.
...
feature-library = Una biblioteca que aguanta
    .body = Un escáner paralelo lee las etiquetas completas, las duraciones reales y el códec propio de cada archivo, no un vistazo a la cabecera. La vigilancia de carpetas sobrevive a los renombrados, y nada desaparece en silencio.
...
feature-similarity = Ordenado por cómo suena
    .body = rox describe tus pistas a partir de su propio audio, tempo incluido, y ordena lo que viene por parecido. Corre en tu máquina sobre tus archivos, sin ningún servicio de por medio.
...
feature-shaders = Visuales que escuchan
    .body = Escribe un shader WGSL sobre un panel o sobre toda la ventana. Señales con nombre sacadas del espectro alimentan sus entradas, así una banda de bombo mueve el mando que tú le señales.
...
download-title = Descargar rox para Linux, macOS o Windows
    .description = Descarga rox gratis para Linux, macOS o Windows. Un binario por plataforma, sin instalador y sin cuenta. Código abierto bajo la AGPL.
    .image-alt = El logo de rox sobre las palabras: descarga para Linux, macOS y Windows

download-h1 = Descargar rox
    .lede = Un binario por plataforma. Sin instalador, sin cuenta, y nada corriendo en segundo plano cuando está cerrado.
...
workspaces-title = Espacios de trabajo de rox - temas y disposiciones que se comparten
    ...
    .image-alt = El logo de rox sobre las palabras: un aspecto entero en un archivo que se comparte
...
platform-closer = Apúntalo a tu biblioteca
    .body = Libre y de código abierto bajo la AGPL. Sin cuenta, y nada corriendo en segundo plano cuando está cerrado.
...
linux-alsa-1 = El modo exclusivo abre la tarjeta directamente como `hw:CARD=x,DEV=n`. Ese es el único nombre de ALSA sin dmix, sin plug y sin ningún servidor de sonido por el camino, que es lo que lo convierte en la única vía que rechazará una frecuencia que no puede dar en vez de remuestrear a otra cosa sin avisar.
...
windows-install-1 = Descomprime donde quieras y ejecuta `rox.exe`. No hay instalador, ni servicio, ni entrada de desinstalación, ni nada corriendo en segundo plano una vez cerrado.
windows-install-2 = El modo portátil guarda la biblioteca, los ajustes y las cachés en una carpeta `rox-data` junto al ejecutable, así que todo cabe en un pendrive. Pasa `--portable` para una ejecución, o deja un archivo vacío llamado `portable` junto al ejecutable para que sea permanente.
...
windows-limit-instance = Sin control de instancia única. En Linux y macOS un segundo arranque le pasa sus archivos al rox que ya corre; en Windows arranca una segunda copia.
...
best-criterion-layout = La disposición tiene que ser tuya
    .body = Las preferencias aquí son genuinamente personales, que es el argumento a favor de la composición frente a un buen valor predeterminado. Paneles que colocas, duplicas con sus propias configuraciones y sacas a ventanas de verdad le ganan a cualquier disposición única que haya elegido otra persona.

best-criterion-yours = Tiene que ser tuyo para quedártelo
...
best-shortlist = La lista corta honesta
    ...
    .p2 = Si quieres algo con un ecosistema grande de plugins y un largo historial en Linux, Quod Libet y Strawberry son respuestas de verdad, y ambos tardan más en cargar una biblioteca de este tamaño de lo agradables que resultan una vez cargada. Si el criterio del etiquetado es el que de verdad te importa, eso está escrito aparte como [alternativa a Mp3tag](/mp3tag-alternative), y el lado de la sonoridad del primer criterio está cubierto en [ReplayGain](/replaygain).
...
musicbee-good = En qué es bueno MusicBee
    ...
    .p2 = También corre en Windows y en ningún otro sitio. Con Wine consigues una versión de él, con la advertencia de siempre: funciona hasta que una actualización decide lo contrario.
...
musicbee-gaps = Lo que rox no hace
    ...
    .close = Cuatro cosas reales y la lista no está rellenada para parecer honesta. Si dos de ellas forman parte de cómo usas un reproductor, para aquí.

musicbee-gap-sync = **Sin sincronización con dispositivos.** Si mueves música a un móvil o a un DAP desde tu reproductor, rox no tiene ninguna respuesta.
...
musicbee-transfers = Qué se traslada, y qué es mejor
    .p1 = La biblioteca aguanta al tamaño que suelen tener los usuarios de MusicBee. Un escáner paralelo lee las etiquetas completas, las duraciones reales y el códec, la frecuencia de muestreo y la profundidad de bits propios de cada archivo en vez de echar un vistazo a las cabeceras, y los archivos con etiquetas ilegibles se indexan por nombre de archivo en vez de desaparecer sin más. La edición de etiquetas es un editor de verdad con edición por lotes, una rejilla por archivo, y una [ruta de escritura que copia y verifica](/mp3tag-alternative) en vez de modificar los originales en el sitio.
    .p2 = La interfaz es la diferencia mayor. MusicBee te da una buena disposición y algo de personalización; rox te da unos cuarenta tipos de panel que colocas tú, duplicas con sus propias configuraciones y sacas a ventanas del sistema de verdad. Un aspecto entero es un archivo con la disposición, la paleta y la apariencia, lo que significa que la organización con la que te quedes es algo que puedes pasarle a otra persona, o llevarte a tu otra máquina, en vez de algo que vive en una instalación.
...
musicbee-split = La división honesta
    .p1 = ¿Te quedas en Windows con una configuración de MusicBee que sincroniza tu móvil y le mete podcasts? Nada de aquí sustituye eso, y cambiar te costaría dos funciones de las que rox no tiene versión.
...
catrox-recreation = La recreación
    .body = El espacio de trabajo CaTRoX de rox es esa disposición reconstruida con paneles nativos: la carátula circular, la lista de campos de metadatos, la lista de pistas agrupada por álbum con sus puntos de valoración, la barra de menús. Es un aspecto, reproducido, no un port de nada. Nada del código de CaTRoX existe en rox y nada podría, porque rox es Rust sobre [gpui](@gpui) y CaTRoX son scripts de panel corriendo dentro de foobar2000. La discusión más amplia con foobar2000, incluidas las filas donde sigue por delante, está en [su propia página](/foobar2000-alternative).
...
catrox-closer = Pruébalo
    .body = El espacio de trabajo CaTRoX viene de serie, a un clic en la ventana de bienvenida. Otros { $others } también, incluidos una [disposición Foobar y una con forma de WinAmp](/workspaces).
...
cue-rows = Filas de verdad, no fragmentos
    .p1 = rox indexa cada tramo del sheet como una fila normal de la biblioteca, identificada por su archivo más su número de pista. Todo lo que hay aguas abajo la lee como una pista, porque para todo lo que hay aguas abajo lo es. Las listas la congelan, las escuchas se le enganchan, la búsqueda la encuentra, las columnas de orden la ordenan, y ninguna sabe que once filas apuntan al mismo FLAC.
...
cue-playing = Reproducir un tramo como un archivo
    .p1 = El motor toma un tramo como el mundo entero de esa pista: una búsqueda exacta hasta su inicio, un recorte con precisión de muestra en los dos extremos, y un límite final que sigue el mismo camino que un final de archivo real. La reproducción sin huecos, el fundido encadenado, el parar tras la actual y la repetición funcionan todos sin saber que existen los tramos.
    .p2 = El recorte de cabeza es el detalle que importa. Una búsqueda exacta aterriza en un límite de paquete y no en la muestra exacta, así que sin descartar los fotogramas entre el aterrizaje y el inicio del tramo, cada pista empieza con la cola de la anterior. Ese es el sonido de una implementación de cue que nadie terminó.
...
fb-does = Qué hace rox al respecto
    .p1 = Nativo en las tres plataformas, un binario en cada una, escrito en Rust sobre [gpui](@gpui). Unos cuarenta tipos de panel que colocas tú, duplicas con configuraciones independientes y sacas a ventanas del sistema de verdad. El escáner de la biblioteca lee las etiquetas completas, las duraciones reales y el códec, la frecuencia de muestreo y la profundidad de bits propios de cada archivo en vez de echar un vistazo a las cabeceras, y aguanta con 50.000 pistas.
...
fb-row-sharing = Compartir un aspecto
    .fb = Una skin son componentes, configuraciones y scripts. Célebremente quisquillosa.
...
fb-row-footprint = Huella
    .fb = Célebremente diminuta.
    .rox = Un binario de 62 MB, 134 MB residentes con 50.000 pistas.

fb-not = Quién no debería cambiar
    .p1 = Si usas Windows, tienes tus componentes configurados y la cosa lleva una década estable, aquí no hay nada que valga el trastorno. Si dependes de un componente concreto de Foobar2000, rox no tiene API de plugins y no puede sustituirlo. Si quieres software que ya haya cometido todos sus errores, rox es cuatro años demasiado joven.
...
mp3tag-write = La ruta de escritura
    .p1 = Esta es la parte que merece leerse aunque nunca instales rox. Casi todos los escritores de etiquetas modifican tu archivo en el sitio: abren el original, reescriben la zona de metadatos y cruzan los dedos. Eso va bien decenas de miles de veces y es catastrófico una, porque un cuelgue, un disco lleno o un cable tirado a medio camino dejan un archivo que ya no se puede leer, con tu audio dentro.
    .p2 = rox nunca escribe dentro de un original. Cada edición copia el archivo, aplica el cambio a la copia, verifica el resultado releyendo los metadatos y calculando el hash del flujo de audio para confirmar que es idéntico byte a byte al que entró, y solo entonces renombra la copia sobre el original de forma atómica. Si algún paso falla, la copia se borra y tu archivo queda intacto, porque nunca llegó a abrirse para escritura.
...
viz-title = Un visualizador de música integrado en el reproductor - rox
    ...
    .image-alt = El logo de rox sobre las palabras: señales sacadas de la música, shaders en todas las superficies
...
viz-shaders = Shaders, en cuatro superficies
    .p1 = Un shader de rox es una etapa de fragmento WGSL. Puede correr sobre el cuerpo de un solo panel, como panel de Shader propio, como superposición sobre toda la ventana, o como fondo pintando bajo todos los paneles, detrás de la carátula. La superposición y el fondo tienen cada uno un interruptor que los extiende a todas las ventanas que rox tenga abiertas. El mismo lenguaje, el mismo bloque de uniforms y el mismo enrutado en todos ellos.
...
viz-limit-milkdrop = WGSL, no Milkdrop ni AVS. No hay importador para ninguno, y veinte años de presets no vienen con nosotros.
...
neko-shot-caption = NekoRoX corriendo en foobar2000, en Windows, en 2022.
...
rg-modes = Ganancia de pista y ganancia de álbum
    ...
    .track = **La ganancia de pista** nivela cada pista por separado. Cada canción llega a la misma sonoridad, que es lo que quieres en aleatorio. También aplana los álbumes construidos con dinámica a propósito: el interludio suave antes del cierre potente sube hasta igualarlo, y el efecto que buscaba el productor desaparece.
    .album = **La ganancia de álbum** aplica una sola cifra a todas las pistas del disco, calculada a partir del álbum entero. Las diferencias relativas dentro del álbum sobreviven exactamente como se masterizaron, y el álbum como unidad queda al nivel de otros álbumes. Este es el valor correcto si escuchas discos de principio a fin.
    .setting = rox lo toma como un ajuste y aplica la cifra elegida por pista según la abre, así que es una preferencia y no algo cocido dentro de los archivos.
...
rg-skip = La parte que casi todas las páginas se saltan
    ...
    .p2 = Esto importa si tienes un DAC que elegiste con cuidado. Bit a bit significa que las muestras que llegan al conversor son las muestras del archivo, sin modificar, así que cualquier cosa que las escale, un control de volumen por debajo de la unidad, un ecualizador, ReplayGain, acaba con la afirmación. No hay ninguna versión de la normalización de sonoridad que deje los bits en paz, porque cambiar la sonoridad es cambiar los bits.
...
rg-closer = Apúntalo a tu biblioteca
    .body = La pasada de medición corre en segundo plano sobre todo lo que no tenga ganancia, y sigue corriendo con la ventana de ajustes cerrada. Más sobre [qué más importa a escala de biblioteca](/best-music-player).
